...
@export var pause_amount: float = 1 # Set to a minimum of 1 but can be increased through editor
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body.name == "Player" or body.has_method("is_player"):
		if body.current_hp > 0:
			...
			# Pause the timer by X amount of time
			var player_hud = get_node("../PlayerHud")
			player_hud.pause_timer(pause_amount)
